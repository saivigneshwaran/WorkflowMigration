...
The helper's Markdown report is a decision report, not a raw log. It classifies SARIF findings into readiness, blocking issues, risks/limitations, activities and packages requiring attention, automated changes, required user actions, approval recommendation, and raw analyzer output. Keep successful parse/restore/test-style messages in the raw analyzer section so they do not obscure migration risks.
