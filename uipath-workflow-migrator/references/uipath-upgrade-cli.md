...
- Helper option `--target-studio-version`: records the Studio version that will open/validate the migrated Windows project, for example `2024.10`, `2025.10`, or `latest STS`.
- CLI option `--outlook-package-version`: Microsoft Office 365 activities package version used for supported mail migration. The bundled CLI README documents default `3.1.21`.

PowerShell helper options use PowerShell casing, for example `-ConsentGated`, `-ProjectPath`, `-OutputPath`, `-TargetStudioVersion 2025.10`, `-ApproveMigration`, `-StatusMode poll`, `-PollIntervalSeconds 60`, `-IncludeRawAnalyzerOutput`, and `-IncludeMigrationGate`.
...
The helper's Markdown report is an assessment report, not a raw log. It classifies SARIF findings and XAML/project inspection into readiness, validation evidence, package-version/Studio compatibility, risk register, blockers, per-risk action guidance, automated changes, owner/remediation guidance, final recommendation, and finding counts. Raw analyzer output and the migration gate section are excluded by default. Include them only when the user explicitly asks for them by passing `-IncludeRawAnalyzerOutput` / `-IncludeMigrationGate` in PowerShell or `--include-raw-analyzer-output` / `--include-migration-gate` in Python.
...
General dependency versions are not invented by the skill. They are determined by the UiPath Upgrade CLI pipeline, migration extensions, configured Studio/Orchestrator/package feeds, and caller-provided CLI options.

For Windows-Legacy conversion, UiPath dependency guidance says to keep the same package version when it exists in configured package sources. If the same version does not exist, conversion selects the highest patch of the nearest available version. If no compatible package exists, report the dependency as unresolved and treat it as a blocker.

The helper records the intended target Studio version with `--target-studio-version` / `-TargetStudioVersion` so the report can tell users where package compatibility must be validated. This does not force dependency versions. Validate the migrated output in the target Studio release and ensure every selected package restores from the same feeds used by the developer and robot.

The main explicit package-version override exposed by the bundled CLI is `--outlook-package-version` for supported Mail/Microsoft 365 migration. Use it when the target Studio environment requires a specific approved Microsoft Office 365 activities package version; otherwise the bundled CLI README documents default `3.1.21`.
